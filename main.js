...
class Admin extends utils.Adapter {
            ...
            if (!err && obj) {
                ...
                if (!obj.native.secret) {
                    require('crypto').randomBytes(24, (ex, buf) => {
                        this.config.secret = buf.toString('hex');
                        this.extendForeignObject('system.config', { native: { secret: this.config.secret } });
                        this.init();
                    });
                } else {
                    this.config.secret = obj.native.secret;
                    this.init();
                }
            } else {
    ...
    updateNews() {
        ...
        return this.getStateAsync('info.newsETag')
            .then(state => {
            ...
            .then(() => (newEtag !== oldEtag ? this.setStateAsync('info.newsETag', newEtag, true) : Promise.resolve()))
    ...
    async checkNodeJsVersion() {
        if (this.supportsFeature('CONTROLLER_NODE_VERSION_CHECK')) {
            return;
        }
        ...
        if (this.instance !== 0) {
            ...
            for (let i = 0; i < objs.rows.length; i++) {
                ...
                if (obj.common.enabled && obj.common.host === this.common.host) {
        ...
        try {
            ...
            const prefix = `system.host.${this.common.host}.versions`;
