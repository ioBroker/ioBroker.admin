...
class Admin extends adapter_core_1.Adapter {
    ...
    onReady = async () => {
        ...
        if (systemConfig) {
            ...
            if (!systemConfig.native.secret) {
                (0, node_crypto_1.randomBytes)(24, (_ex, buf) => {
                    this.secret = buf.toString('hex');
                    this.extendForeignObject('system.config', { native: { secret: this.secret } }).catch(e => this.log.error(`Cannot set secret: ${e}`));
                    this.init();
                });
            }
            else {
    ...
    onMessage = (obj) => {
        ...
        if (obj.command === 'im') {
            ...
            const dockerManager = new plugin_docker_1.DockerManager({
                logger: {
    ...
    async updateNews() {
        ...
        let etag;
        try {
            const res = await axios_1.default.get('https://iobroker.live/repo/news-hash.json', {
                timeout: 13_000,
        ...
        if (etag && etag.hash !== oldEtag) {
            newEtag = etag.hash;
            try {
                const res = await axios_1.default.get('https://iobroker.live/repo/news.json', {
                    timeout: 14_000,
                    validateStatus: status => status < 400,
                });
                _newNews = res.data;
    ...
    init() {
        ...
        void this.updateNews().catch(e => this.log.error(`Cannot update news: ${e.response ? e.response.data : e.message || e.code}`));
