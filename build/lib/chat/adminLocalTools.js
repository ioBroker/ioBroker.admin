...
exports.AUTO_RUN_TOOLS = exports.NAVIGATION_TOOL_DEFS = exports.ADMIN_LOCAL_TOOL_DEFS = exports.ALWAYS_CONFIRM_TOOLS = exports.CLIENT_ACTION_TOOLS = exports.ADMIN_WRITE_TOOLS = exports.MCP_WRITE_TOOLS = void 0;
exports.isAdminLocalTool = isAdminLocalTool;
exports.toolKind = toolKind;
exports.executeAdminLocalTool = executeAdminLocalTool;
/**
...
exports.MCP_WRITE_TOOLS = new Set(['set_state', 'set_states']);
/** Admin-local tools executed in the backend (need confirmation). */
exports.ADMIN_WRITE_TOOLS = new Set([
    'create_user_state',
    'extend_object',
    'assign_to_enums',
    'run_node_script',
...
exports.CLIENT_ACTION_TOOLS = new Set(['install_adapter', 'navigate_admin_ui', 'run_command']);
/**
 ...
 */
exports.ALWAYS_CONFIRM_TOOLS = new Set(['run_node_script', 'run_javascript']);
/** True if the tool is implemented here (admin-local) rather than by the MCP server. */
function isAdminLocalTool(name) {
    return exports.ADMIN_WRITE_TOOLS.has(name) || exports.CLIENT_ACTION_TOOLS.has(name);
}
/** Classify a tool by name to decide whether it needs confirmation and where it runs. */
function toolKind(name) {
    if (exports.CLIENT_ACTION_TOOLS.has(name)) {
        return 'action';
    }
    if (exports.MCP_WRITE_TOOLS.has(name) || exports.ADMIN_WRITE_TOOLS.has(name)) {
        return 'write';
    }
    return 'read';
}
/** OpenAI function-tool definitions for the admin-local tools (added in "act" mode). */
exports.ADMIN_LOCAL_TOOL_DEFS = [
    {
        type: 'function',
        function: {
            name: 'create_user_state',
            description: 'Create a new state (datapoint), restricted to the user namespaces "0_userdata.0." or ' +
    ...
    },
    {
        ...
        function: {
            name: 'extend_object',
            description: 'Update an EXISTING ioBroker object by merging the given `common` and/or `native` into ' +
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Categorize objects by ADDING them to room and/or function enums (`enum.rooms.*` / ' +
                    ...
                    assignments: {
                        ...
                        items: {
                            ...
                            properties: {
                                id: {
                                    type: 'string',
                                    description: 'Full object ID of the device/channel/state, e.g. "zigbee.0.00158d0002e2e668"',
                                },
                                rooms: {
                                    type: 'array',
                                    items: { type: 'string' },
                                ...
                                functions: {
                                    type: 'array',
                                    items: { type: 'string' },
                                    description: 'Function enum ids or names, e.g. ["Light"] or ["enum.functions.motion"]',
                                },
                            },
                            required: ['id'],
                        },
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Execute a short Node.js script on the ioBroker HOST (the OS) to CHECK/diagnose something — ' +
    ...
    },
    {
        ...
        function: {
            name: 'run_javascript',
            description: 'Run a one-off JavaScript or TypeScript script INSIDE the javascript adapter, with the full ' +
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Propose installing an ioBroker adapter from the repository. The installation is handed ' +
    ...
    },
    {
        ...
        function: {
            name: 'run_command',
            description: 'Run an ioBroker CLI command on the current host (the leading "iobroker" is implied). ' +
 ...
 */
exports.NAVIGATION_TOOL_DEFS = [
    {
        type: 'function',
        function: {
            name: 'navigate_admin_ui',
            description: 'Navigate the admin UI directly by setting the URL hash — opens a tab OR a specific dialog ' +
...
exports.AUTO_RUN_TOOLS = new Set(['navigate_admin_ui']);
...
const ok = (data, clientAction) => ({
    text: JSON.stringify({ ok: true, data }),
    isError: false,
    ...(clientAction ? { clientAction } : {}),
});
const fail = (error) => ({ text: JSON.stringify({ ok: false, error }), isError: true });
...
async function executeAdminLocalTool(adapter, name, args) {
    try {
...
async function createUserState(adapter, args) {
    ...
    const type = args.type || 'mixed';
    const common = {
        name: args.name || id.split('.').pop() || id,
        type,
...
function enumNames(common) {
    const name = common?.name;
    if (!name) {
        return [];
    }
    return typeof name === 'string' ? [name] : Object.values(name).filter((n) => typeof n === 'string');
}
/**
 ...
 */
async function assignToEnums(adapter, args) {
    ...
    const resolveEnum = (category, ref) => {
        ...
        if (!working[id]) {
            working[id] =
                enumObjs[id] ||
                    { _id: id, type: 'enum', common: { name: ref, members: [] }, native: {} };
...
function runNodeScript(args) {
    ...
    return new Promise(resolve => {
        (0, node_child_process_1.execFile)(process.execPath, ['-e', code], { timeout, maxBuffer: 4 * 1024 * 1024, windowsHide: true }, (error, stdout, stderr) => {
            resolve(ok({
...
function formatScriptLogs(logs) {
    ...
    return logs.slice(0, 100).map(entry => {
        ...
        if (entry && typeof entry === 'object') {
            ...
            const msg = typeof o.message === 'string' ? o.message : typeof o.text === 'string' ? o.text : JSON.stringify(o);
 ...
 */
async function runJavaScript(adapter, args) {
    const source = argStr(args.source);
    if (!source.trim()) {
        return fail('source is required');
    }
    const lang = argStr(args.language).toLowerCase();
    const engineType = lang === 'ts' || lang === 'typescript' ? 'TypeScript/ts' : 'Javascript/js';
    const timeout = Math.min(Math.max(Number(args.timeout) || 10000, 500), 60000);
    ...
    if (!instance) {
        ...
        if (!ids.length) {
            return fail('The "javascript" adapter is not installed. Offer to install it (install_adapter "javascript") and add an instance.');
        }
    ...
    const result = await Promise.race([
        adapter.sendToAsync(instance, 'execute', message),
        new Promise(resolve => setTimeout(() => resolve({ ok: false, error: `No response from ${instance} within ${guardMs} ms (is it running?)` }), guardMs)),
    ]);
    const r = (result || {});
...
function installAdapter(args) {
    ...
    return ok({ adapter: adapterName, handedToUi: true, note: 'The admin UI will run the installation.' }, { type: 'install', adapter: adapterName });
}
/** Validate the navigation target (a hash route) and hand it to the frontend, which sets the URL. */
function navigateAdminUi(args) {
...
function runCommand(args) {
    ...
    return ok({ command, handedToUi: true, note: 'The command runs in the admin UI; its live output is shown to the user.' }, { type: 'command', command });
}
//# sourceMappingURL=adminLocalTools.js.map