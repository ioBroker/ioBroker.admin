import path, { posix, basename, dirname, resolve, join, sep, extname, parse } from 'path';
        ...
        async transform(code, id) {
            ...
            if (builderInfo.isRemote) {
                ...
                if (id === '\0virtual:__remoteEntryHelper__') {
                    for (const expose of parsedOptions.prodExpose) {
                        code = code.replace(`\${__federation_expose_${expose[0]}}`, `./${basename(this.getFileName(expose[1].emitFile))}`);
                    }
                    return code;
                }
            }
