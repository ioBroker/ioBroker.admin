...
function IOSocket(server, settings, adapter, objects, states, store) {
    ...
    function socketEvents(socket) {
        ...
        socket.on('disconnect', function () {
            ...
            if (adapter.requireLog) {
                adapter.requireLog(!!that.io.sockets.sockets.length);
            }
        });
    ...
    function disableEventThreshold(readAll) {
        if (eventsThreshold.active) {
            ...
            setTimeout(function () {
                    ...
                    adapter.getForeignStates('*', function (err, res) {
                        ...
                        for (var id in res) {
                            if (res.hasOwnProperty(id) && JSON.stringify(states[id]) !== JSON.stringify(res[id])) {
                                that.io.sockets.emit('stateChange', id, res[id]);
                                states[id] = res[id];
                            }
                        }
                    });
                ...
                that.io.sockets.emit('eventsThreshold', false);
    ...
    function enableEventThreshold() {
        if (!eventsThreshold.active) {
            ...
            setTimeout(function () {
                ...
                that.io.sockets.emit('eventsThreshold', true);
    ...
    this.repoUpdated = function () {
        if (that.io && that.io.sockets) {
            that.io.sockets.emit('repoUpdated');
        }
    };

    this.objectChange = function (id, obj) {
        var clients = that.io.sockets.connected;
        ...
        that.io.sockets.emit('objectChange', id, obj);
    };

    this.sendCommand = function (obj) {
        if (cmdSessions[obj.message.id]) {
            if (that.io) {
                that.io.sockets.emit(obj.command, obj.message.id, obj.message.data);
            }
    ...
    this.stateChange = function (id, state) {
        ...
        that.io.sockets.emit('stateChange', id, state);
    };
    ...
    var __construct = (function () {
        ...
        that.io = socketio.listen(server, (settings.bind && settings.bind !== '0.0.0.0') ? settings.bind : undefined);

        that.io.on('connection', initSocket);

        if (settings.auth) {
            that.io.use(passportSocketIo.authorize({
                passport:     passport,
