...
function IOSocket(server, settings, adapter, objects, store) {
    ...
    async function rename(_adapter, oldName, newName, options) {
        ...
        try {
            ...
            const files = await adapter.readDirAsync(_adapter, oldName, options);
            if (files && files.length) {
                for (let f = 0; f < files.length; f++) {
                    await rename(_adapter, oldName + '/' + files[f].file, newName + '/' + files[f].file);
                }
            }
        } catch (error) {
        ...
        try {
            await adapter.renameAsync(_adapter, oldName, newName, options);
        } catch (error) {
            if (error.message !== 'Not exists') {
                throw error;
            }
    ...
    async function unlink(_adapter, name, options) {
        // read if it is a file or folder
        try {
            // remove trailing '/'
            if (name.endsWith('/')) {
            ...
            const files = await adapter.readDirAsync(_adapter, name, options);
            if (files && files.length) {
                for (let f = 0; f < files.length; f++) {
                    await unlink(_adapter, name + '/' + files[f].file);
                }
            }
        } catch (error) {
        ...
        try {
            await adapter.unlinkAsync(_adapter, name, options);
        } catch (error) {
            if (error.message !== 'Not exists') {
                throw error;
            }
    ...
    function socketEvents(socket, cb) {
            ...
            if (updateSession(socket) && checkPermissions(socket, 'getState', callback, id)) {
                if (typeof callback === 'function') {
                    if (adapter.getForeignBinaryState) {
                        adapter.getForeignBinaryState(id, (err, data) => {
                            if (data) {
            ...
            if (updateSession(socket) && checkPermissions(socket, 'setState', callback, id)) {
                if (typeof callback === 'function') {
                    ...
                    if (adapter.setForeignBinaryState) {
                        adapter.setForeignBinaryState(id, data, (err, ...args) =>
                            fixCallback(callback, err, ...args));
                    } else {
        ...
        socket.on('deleteFolder', function (_adapter, filename, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'deleteFolder', callback, filename)) {
                unlink(_adapter, filename, {user: this._acl.user})
        ...
        socket.on('decrypt', function (encryptedText, callback) {
            if (that.secret) {
                typeof callback === 'function' && callback(null, adapter.decrypt(that.secret, encryptedText));
            } else {
        ...
        socket.on('encrypt', function (plainText, callback) {
            if (that.secret) {
                typeof callback === 'function' && callback(null, adapter.encrypt(that.secret, plainText));
            } else {
                adapter.getForeignObject('system.config', (err, obj) => {
                    if (obj && obj.native && obj.native.secret) {
                        that.secret = obj.native.secret;
    ...
    function onAuthorizeFail(data, message, error, accept) {
        ...
        error && adapter.log.info(`failed connection to socket.io from ${data.connection.remoteAddress}:`, message);
                    ...
                    if (systemLicenses && systemLicenses.native && systemLicenses.native.password && systemLicenses.native.login) {
                        ...
                        return adapter.getForeignObjectAsync('system.config')
                            .then(systemConfig => {
                                ...
                                let password;
                                try {
                                    password = adapter.decrypt(systemConfig.native.secret, systemLicenses.native.password);
                                } catch {
    ...
    (function __constructor() {
        ...
        that.server.on('error', error => {
            ...
            if (!error || !error.message || !error.message.includes('failed connection')) {
                adapter.log.error(`Error: ${(error && error.message) || JSON.stringify(error)}`);
            }
        });
