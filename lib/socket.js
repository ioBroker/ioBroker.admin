...
class AdminSocket extends IOSocket {
    constructor(server, settings, adapter, objects, store, checkUser) {
        super(server, settings, adapter, objects, store, checkUser, false, true);
        ...
        this.__init(server, {checkUser, userKey: 'connect.sid'});
    }
            ...
            if (this.objects) {
                if (socket._acl &&
                    ...
                    for (const id in this.objects) {
                        if (this.objects.hasOwnProperty(id) && AdminSocket._checkObject(this.objects[id], socket._acl, 4 /* 'read' */)) {
                ...
                this.adapter.getObjectList({include_docs: true}, (err, res) => {
                    ...
                    if (socket._acl &&
                        ...
                        for (let i = 0; i < res.length; i++) {
                            if (AdminSocket._checkObject(res[i].doc, socket._acl, 4 /* 'read' */)) {
    ...
    _initCommandsUser() {
        this.commands['addUser'] = (socket, user, pass, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'addUser', callback, user)) {
        ...
        this.commands['delUser'] = (socket, user, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'delUser', callback, user)) {
        ...
        this.commands['addGroup'] = (socket, group, desc, acl, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'addGroup', callback, group)) {
        ...
        this.commands['delGroup'] = (socket, group, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'delGroup', callback, group)) {
    ...
    _initCommandsAdmin() {
        this.commands['getHostByIp'] = (socket, ip, callback) => {
            ...
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'getHostByIp', ip)) {
        ...
        this.commands['requireLog'] = (socket, isEnabled, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'setObject', callback)) {
                if (isEnabled) {
        ...
        this.commands['readLogs'] = (socket, host, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'readLogs', callback)) {
        ...
        this.commands['delState'] = (socket, id, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'delState', callback, id)) {
                // clear cache
        ...
        this.commands['cmdExec'] = (socket, host, id, cmd, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'cmdExec', callback, cmd)) {
            ...
            if (this.secret) {
                ...
                this.adapter.getForeignObject('system.config', (err, obj) => {
                    if (obj && obj.native && obj.native.secret) {
                        ...
                        IOSocket._fixCallback(callback, err);
                    }
                });
            }
        };
        ...
        this.commands['getIsEasyModeStrict'] = (socket, callback) => {
            typeof callback === 'function' && callback(null, this.settings.accessLimit);
        };

        this.commands['getEasyMode'] = (socket, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'getObject', callback)) {
                let user;
                if (this.settings.auth) {
                    ...
                    user = this.settings.defaultUser;
                }

                if (this.settings.accessLimit) {
                    const configs = [];
                    const promises = [];
                    this.settings.accessAllowedConfigs.forEach(id => promises.push(this._readInstanceConfig(id, user, false, configs)));
                    this.settings.accessAllowedTabs.forEach(id    => promises.push(this._readInstanceConfig(id, user, true, configs)));
        ...
        this.commands['getAdapters'] = (socket, adapterName, callback) => {
            if (typeof callback === 'function' && this.__updateSession(socket) && this._checkPermissions(socket, 'getObject', callback)) {
        ...
        this.commands['updateLicenses'] = (socket, login, password, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'setObject', callback, login, password)) {
        ...
        this.commands['getCompactInstalled'] = (socket, host, callback) => {
            if (typeof callback === 'function') {
                if (this.__updateSession(socket) && this._checkPermissions(socket, 'sendToHost', callback)) {
    ...
    __initCommandsStates() {
        ...
        this.commands['getForeignStates'] = (socket, pattern, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'getStates', callback)) {
                if (typeof callback === 'function') {
    ...
    __initCommandsObjects() {
        ...
        this.commands['extendObject'] = (socket, id, obj, callback) => {
            if (this.__updateSession(socket) && this._checkPermissions(socket, 'extendObject', callback, id)) {
