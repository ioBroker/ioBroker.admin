...
function IOSocket(server, settings, adapter, objects, store) {
    ...
    function socketEvents(socket, cb) {
        ...
        socket.on('getCountsOfInstances', function (callback) {
            if (typeof callback === 'function') {
                if (updateSession(socket) && checkPermissions(socket, 'getObject', callback)) {
                            ...
                            if (err) {
                                ...
                                doc.rows.forEach(item => {
                                    ...
                                    if (obj && obj.common && obj.common.name) {
                                        result[obj.common.name] = result[obj.common.name] || 0;
                                        result[obj.common.name]++;
                                    }
                                });
        ...
        socket.on('getIgnoredVersion', function (callback) {
            if (typeof callback === 'function') {
                if (updateSession(socket) && checkPermissions(socket, 'getObject', callback)) {
                    adapter.getObjectView('system', 'adapter',
                        ...
                        (err, doc) => {
                            if (err) {
                                ...
                                doc.rows.forEach(item => {
                                    const obj = item.value;
                                    if (obj && obj.common && obj.common.name && obj.common.ignoreVersion) {
                                        result[obj.common.name] = obj.common.ignoreVersion;
                                    }
                                });
        ...
        socket.on('getCompactInstalled', function (callback) {
            if (typeof callback === 'function') {
                if (updateSession(socket) && checkPermissions(socket, 'getObject', callback)) {
                    adapter.getObjectView('system', 'adapter',
                        {startkey: `system.adapter.`, endkey: `system.adapter.\u9999`},
        ...
        socket.on('getSystemConfigCommon', function (callback) {
            if (updateSession(socket) && checkPermissions(socket, 'getObject', callback)) {
                adapter.getForeignObject('system.config', (err, obj) => {
                    delete obj.native;
                    callback(err,  obj);
