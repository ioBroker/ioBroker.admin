    ...
    function socketEvents(socket, cb) {
        ...
        socket.on('delObject', function (id, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'delObject', callback, id)) {
                adapter.delForeignObject(id, {user: this._acl.user}, (err, ...args) =>
                    fixCallback(callback, err, ...args));
            }
        });

        socket.on('delObjects', function (id, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'delObject', callback, id)) {
                adapter.delForeignObject(id, {user: this._acl.user, recursive: true}, (err, ...args) =>
                    fixCallback(callback, err, ...args));
            }
        });
