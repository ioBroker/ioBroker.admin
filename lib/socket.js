...
function IOSocket(server, settings, adapter, objects, store) {
    ...
    this.subscribe = function (socket, type, pattern) {
        ...
        if (p === null) {
            return adapter.log.warn('Empty pattern!');
        }
        if (socket) {
    ...
    function socketEvents(socket) {
        ...
        socket.on('authenticate', function (cb) {
        ...
        socket.on('setBinaryState', function (id, base64, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'setState', callback, id)) {
                if (typeof callback === 'function') {
                    ...
                    adapter.setBinaryState(id, data, callback);
                } else {
        ...
        socket.on('writeFile', function (_adapter, filename, data64, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'writeFile', callback, filename)) {
                ...
                adapter.writeFile(_adapter, filename, buffer, {user: this._acl.user}, callback);
            }
        });
        ...
        socket.on('writeFile64', function (_adapter, fileName, data64, options, callback) {
            ...
            if (updateSession(socket) && checkPermissions(socket, 'writeFile64', callback, fileName)) {
                ...
                let buffer = new Buffer(data64, 'base64');
                adapter.writeFile(_adapter, fileName, buffer, options, callback);
            }
        });
        ...
        socket.on('decryptPhrase', function (encryptedPhrase, callback) {
            if (that.secret) {
                typeof callback === 'function' && adapter.tools.decryptPhrase(that.secret, encryptedPhrase, plainText =>
        ...
        socket.on('encryptPhrase', function (plainTextPhrase, callback) {
            if (that.secret) {
                typeof callback === 'function' && adapter.tools.encryptPhrase(that.secret, plainTextPhrase, encrypted =>
    ...
    (function __constructor() {
        ...
        if (!settings.clientid) {
            if (!server.__inited) {
                ...
                that.server = socketio.listen(server/*, {serveClient: false}*/);
