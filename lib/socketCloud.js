    ...
    _socketEvents(socket, address, cb) {
        return super._socketEvents(socket, address, () => {
            ...
            socket.on('cloudCommand', (cmd, data) => {
                if (cmd === 'stop') {
                    this._stopAdapter(data);
                } else if (cmd === 'redirect') {
                    this._redirectAdapter(data);
                } else if (cmd === 'wait') {
                    this._waitForConnect(data || 30);
                }
            });
