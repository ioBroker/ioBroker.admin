...
class SocketAdmin extends SocketCommon {
    ...
    _onAuthorizeFail = (data, message, error, accept) => {
        ...
        error &&
            this.adapter.log.info(`failed connection to socket.io from ${data.connection.remoteAddress}: ${message}`);
    ...
    __updateSession(socket) {
        if (socket._sessionID) {
            const time = Date.now();
            if (socket._lastActivity && time - socket._lastActivity > this.settings.ttl * 1_000) {
