...
class SocketCommon extends EventEmitter {
    static COMMAND_RE_AUTHENTICATE = 'reauthenticate';

    constructor(settings, adapter, objects, store, clientMode) {
        super();

        this.store        = store || settings.store;
    ...
    start(server, socketClass, authOptions, socketOptions) {
        if (!socketClass) {
            throw new Error('"socketClass" not provided by start');
        }
    ...
    static _getPermissionsForIp(address, whiteList) {
        return whiteList[SocketCommon._getWhiteListIpForAddress(address, whiteList) || 'default'];
    }
    ...
    publish(socket, type, id, obj) {
        this.commands.publish(socket, type, id, obj);
    }
