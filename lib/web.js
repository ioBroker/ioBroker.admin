    ...
    function prepareIndex() {
        ...
        m.forEach(pattern => {
            ...
            if (pattern === 'loginBackgroundImage') {
                ...
                template = template.replace('@@loginBackgroundColor@@', adapter.config.loginBackgroundColor || 'inherit');
            } else
            if (pattern === 'loadingBackgroundImage') {
                if (adapter.config.loadingBackgroundImage) {
                    template = template.replace('@@loadingBackgroundImage@@', `files/${adapter.namespace}/loading-bg.png`);
                } else {
                ...
                if (server.server) {
                    ...
                    adapter.getPort(settings.port, (!settings.bind || settings.bind === '0.0.0.0') ? undefined : settings.bind || undefined, port => {
                        ...
                        server.server.listen(port, (!settings.bind || settings.bind === '0.0.0.0') ? undefined : settings.bind || undefined, () => {
                            ...
                            adapter.log.info(`Use link "http${settings.secure ? 's' : ''}://127.0.0.1:${port}" to configure.`);
                        });
