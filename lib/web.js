            ...
            if (settings.auth) {
                ...
                passport.use(new LocalStrategy((username, password, done) => {
                    ...
                    adapter.checkPassword(username, password, (res, user) => {
                        ...
                        if (res) {
                            return done(null, user.replace(/^system\.user\./, ''));
                        } else {
                    ...
                    passport.authenticate('local', (err, user, info) => {
                        ...
                        req.logIn(user, err => {
                            ...
                            if (req.body.stayLoggedIn) {
                                req.session.cookie.maxAge = new Date(Date.now() + (settings.ttl > ONE_MONTH_SEC ? settings.ttl : ONE_MONTH_SEC) * 1000);
                                req.session.cookie.httpOnly = true;
                            } else {
                                req.session.cookie.httpOnly = true;
                                req.session.cookie.maxAge = new Date(Date.now() + settings.ttl * 1000);
                            }
