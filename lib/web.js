            ...
            server.app.use('/files/', async (req, res) => {
                ...
                try {
                    if (await adapter.fileExists(adapterName, url)) {
                        ...
                        adapter.log.warn('adaptr' + adapterName)
                        adapter.log.warn('url' + url)
                        ...
                        for (const fileMeta of fileMetas) {
                            if (!fileMeta.isDir) {
                                ...
                                if (file.file instanceof Buffer) {
                                    filesOfDir.push( {name: fileMeta.file, file: file.file })
                                } else {
                                    filesOfDir.push({ name: fileMeta.file, file: Buffer.from(file.file, 'utf-8') } )
                                }
                            } else {
