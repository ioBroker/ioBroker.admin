                    ...
                    if (!req.isAuthenticated()) {
                        if (/admin\.\d+\/login-bg\.png(\?.*)?$/.test(req.originalUrl)) {
                            ...
                            if (adapter.config.react) {
                                ...
                                if (!req.url.startsWith('/?') && !unprotectedFiles.find(file => req.url.startsWith('/' + file.name + (file.isDir ? '/' : '')))) {
                        ...
                        // This code seems to be "dead" at least with socket.io 2.3.0 because as soon as socket.io
