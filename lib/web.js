        ...
        m.forEach(pattern => {
            ...
            if (pattern === 'loginBackgroundImage') {
                if (adapter.config[pattern]) {
                    template = template.replace(`@@loginBackgroundImage@@`, adapter.namespace + '/login-bg.png');
                } else {
                    template = template.replace(`@@loginBackgroundImage@@`, '');
                }
            } else if (pattern === 'loginBackgroundColor') {
                template = template.replace(`@@loginBackgroundColor@@`, adapter.config[pattern]);
            } else if (pattern === 'vendorPrefix') {
                template = template.replace(`@@vendorPrefix@@`, uuid.length > 36 ? uuid.substring(0, 2) : '');
            } else {
