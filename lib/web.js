...
function Web(settings, adapter, onReady, options) {
    ...
    let LOGIN_PAGE = '/index.html?login';
    ...
    function prepareIndex() {
        ...
        m.forEach(pattern => {
            ...
            if (pattern === 'disableDataReporting') {
                template = template.replace(/['"]@@disableDataReporting@@["']/g, adapter.common.disableDataReporting ? 'true' : 'false');
            } else
            if (pattern === 'loginBackgroundImage') {
                if (adapter.config.loginBackgroundImage) {
                    template = template.replace(/@@loginBackgroundImage@@/g, `files/${adapter.namespace}/login-bg.png`);
                } else {
                    template = template.replace(/@@loginBackgroundImage@@/g, '');
                }
            } else if (pattern === 'loginBackgroundColor') {
                template = template.replace(/@@loginBackgroundColor@@/g, adapter.config.loginBackgroundColor || 'inherit');
            } else
            if (pattern === 'loadingBackgroundImage') {
                if (adapter.config.loadingBackgroundImage) {
                    template = template.replace(/@@loadingBackgroundImage@@/g, `files/${adapter.namespace}/loading-bg.png`);
                } else {
                    template = template.replace(/@@loadingBackgroundImage@@/g, '');
                }
            } else if (pattern === 'loadingBackgroundColor') {
                template = template.replace(/@@loadingBackgroundColor@@/g, adapter.config.loadingBackgroundColor || '');
                ...
                server.app.use((req, res, next) => {
                    ...
                    if (!req.isAuthenticated()) {
                        if (/^\/login\//.test(req.originalUrl) ||
                            ...
                            unprotectedFiles = unprotectedFiles || fs.readdirSync(path.join(__dirname, `../${wwwDir}/`)).map(file => {
                ...
                if (url.startsWith(`/${adapter.name}/`)) {
                    ...
                    if (url.startsWith(dirName)) {
                        try {
                            ...
                            res.status(404).send(`File not found: ${escapeHtml(JSON.stringify(e))}`);
                        }
                    } else {
