...
function Web(settings, adapter, onReady) {
            ...
            adapter.getForeignObject('system.adapter.' + id, {user}, (err, obj) => {
                if (obj && obj.common) {
                    ...
                    const config = {
                        ...
                        url: '/adapter/' + obj.common.name + '/index' + (obj.common.materialize ? '_m' : '') + '.html' + (instance ? '?' + instance : ''),
    ...
    function generateConfigPage(req, res) {
        if (!configPage) {
            configPage = fs.existsSync(__dirname + '/../src/configs.html') ?
            ...
            if (settings.limitAccess) {
                const configs = [];
                const promises = [];
                settings.allowedConfigs.forEach(id => promises.push(readInstanceConfig(id, user, false, configs)));
                settings.allowedTabs.forEach(id    => promises.push(readInstanceConfig(id, user, true, configs)));

                Promise.all(promises)
                    .then(() => {
                        configPage = configPage.replace('%%CONFIG%%', JSON.stringify(configs));
                        res.send(configPage);
                    });
            } else {
    ...
    (function __construct () {
        if (settings.port) {
            ...
            settings.allowedConfigs = settings.allowedConfigs || [];
            settings.allowedTabs    = settings.allowedTabs || [];
            ...
            server.app.use('/adapter/', (req, res) => {
                ...
                if (settings.limitAccess) {
                    if (url === 'index.html' || url === 'index_m.html') {
                        const anyConfig = settings.allowedConfigs.includes(adapterName + '.' + _instance);
                        if (!anyConfig) {
                            res.contentType('text/html');
                    ...
                    if (url === 'tab.html' || url === 'tab_m.html') {
                        const anyTabs = settings.allowedTabs.includes(adapterName + '.' + _instance);
                        if (!anyTabs) {
                            res.contentType('text/html');
                            return res.status(403).send('You are not allowed to access this page');
                        }
