...
async function readFolderRecursive(adapter, adapterName, url, _result, _baseDir) {
    _result = _result || [];
    const fileMetas = await this.adapter.readDirAsync(adapterName, url);
    for (const fileMeta of fileMetas) {
        if (!fileMeta.isDir) {
            const file = await this.adapter.readFileAsync(adapterName, `${url}/${fileMeta.file}`);
            const name = _baseDir ? `${_baseDir}/${fileMeta.file}` : fileMeta.file;
            if (file.file instanceof Buffer) {
                _result.push({ name, file: file.file });
            } else {
                _result.push({ name, file: Buffer.from(file.file, 'utf-8') });
            }
        } else {
            if (_baseDir) {
                _baseDir = `${_baseDir}/${fileMeta.file}`;
            } else {
                _baseDir = fileMeta.file;
            }
            await readFolderRecursive(adapter, adapterName, `${url}/${fileMeta.file}`, _result, _baseDir);
        }
    }
...
/** @typedef {{ port: number, auth: boolean, secure: boolean, bind: string, cache: boolean}} WebSettings */
    ...
    async init() {
        if (this.settings.port) {
            ...
            this.server.app.get('*/_socket/info.js', (req, res) => {
                ...
                res.status(200).send(this.getInfoJs(this.settings));
            });
            ...
            .then(obj => {
                ...
                if (this.server.server) {
                    ...
                    this.adapter.getPort(
                        ...
                        port => {
                            if (port !== this.settings.port && !this.adapter.config.findNextPort) {
                                this.adapter.log.error(`port ${this.settings.port} already in use`);
                                return this.adapter.terminate
                                    ? this.adapter.terminate(
                                          `port ${this.settings.port} already in use`,
