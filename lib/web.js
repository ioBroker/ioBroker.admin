    ...
    function disableTar() {
        ...
        try {
            ...
            if (config &&
                ...
                config.log.transport.file1) {
                // If zippedArchive is not yet disabled
                if (config.log.transport.file1.zippedArchive === undefined || config.log.transport.file1.zippedArchive === true) {
                    adapter.log.error('Cannot unzip log archive, the compress feature was disabled. Please restart ioBroker!');
                    config.log.transport.file1.zippedArchive = false;
                    fs.writeFileSync(configFile, JSON.stringify(config, null, 2));
                } else {
                    adapter.log.error('Cannot unzip log archive, the compress feature is yet disabled. May be controller not yet restarted!');
                }
            } else {
                adapter.log.error('Cannot unzip log archive, but the config is old and zippedArchive cannot be disabled! Please do it manually.');
            }
        } catch (e) {
            adapter.log.error('Cannot parse ' + configFile + ' to disable zippedArchive');
        }
    }
            ...
            if (settings.auth) {
                ...
                server.app.use((req, res, next) => {
                    ...
                    if (!req.isAuthenticated()) {
                        ...
                        // @Bluefox: I was not able to create one case where this code was used at all ...
                        // This code seems to be "dead" at least with socket.io 2.3.0 because as sopn as socket.io
                        // is added it catches all requests to "/socket.io" in path.
                        // Do you remember the reason for this code?
                        if (socketIoFile !== false && (req.url.startsWith('socket.io.js') || req.url.match(/\/socket\.io\.js(\?.*)?$/))) {
                            if (socketIoFile) {
                                ...
                                try {
                                    ...
                                    // ... and then from their (with normally unneeded faillback to "us")
                                    // we lookup the client library
                                    const clientDir = require.resolve('socket.io-client', {
                                       paths: [path.dirname(socketIoDir), __dirname]
                                    });
                                    const fileDir = path.join(path.dirname(clientDir), '../dist/');
                                    ...
                                    try {
                                        // as fallback (should never happen!) we return a local version
        ...
        if (server.server) {
            ...
            adapter.getPort(settings.port, port => {
                if (port !== settings.port && !adapter.config.findNextPort) {
                    ...
                    return adapter.terminate ? adapter.terminate('port ' + settings.port + ' already in use', 1) : process.exit(1);
                }
                ...
                try {
                    server.server.on('error', e => {
                        ...
                        return adapter.terminate ? adapter.terminate(e, 1) : process.exit(1);
                    });
                    ...
                    adapter.terminate ? adapter.terminate(e, 1) : process.exit(1);
                }
