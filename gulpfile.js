...
function build() {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!fs.existsSync(script)) {
            ...
            const cmd = `node --max-old-space-size=7000 ${script} build`;
...
gulp.task('react-0-configCSS', () => {
    return gulp.src([
        `./${srcRx}less/selectID.less`,
        `./${srcRx}less/adapter.less`,
        `./${srcRx}less/materializeCorrect.less`
    ])
...
gulp.task('react-0-iobCSS', () => {
    return gulp.src([`./${srcRx}less/selectID.less`])
        .pipe(sourcemaps.init())
        .pipe(less({
            paths: [ ]
        }))
...
gulp.task('react-0-treeTableCSS', () => {
    return gulp.src([`./${srcRx}less/jquery.treetable.theme.less`])
        .pipe(sourcemaps.init())
        .pipe(less({
            paths: [ ]
        }))
