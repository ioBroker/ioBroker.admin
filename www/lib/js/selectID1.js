...
(function ($) {
    ...
    function initTreeDialog($dlg, resort) {
        ...
        text += '<div style="width: 100%; height: ' + (data.buttons ? 'calc(100% - 30px)' : 'calc(100% - 30px)') + '; padding:0; overflow-y: scroll">';
                        ...
                        case 'enum':
                            if (isCommon && obj.common.members && obj.common.members.length > 0) {
                                if (obj.common.members.length < 4) {
                                    $tdList.eq (base).text ('(' + obj.common.members.length + ')' + obj.common.members.join (', '));
                                } else {
                                    $tdList.eq (base).text (obj.common.members.length);
                                }
                                $tdList.eq (base).attr ('title', obj.common.members.join ('\x0A'));
                            } else {
                                $tdList.eq (base).text ('');
