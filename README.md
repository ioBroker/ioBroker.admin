...
- (@GermanBluefox) Added: an assistant in the "Enums" tab sets up floors, rooms and functions step by step. Floors become rooms, which contain the rooms of the floor. Rooms and functions are chosen from the templates or added by name, existing entries stay unchanged. After creating them, the devices are assigned in the embedded assignment view, and an overview shows the result. The assistant opens by itself once, as long as there are no rooms at all
- (@GermanBluefox) Added: the "Enums" tab has a second view "Assignment". It lists the devices and channels with their rooms and functions side by side, with filters for the objects without a room or without a function and for the adapter instance. A plus button in every cell adds or removes a room or function, and several objects can be set at once. Assignments over the device or over single states are shown too. From the device type, which `@iobroker/type-detector` recognizes, a matching function is suggested and applied with one click
- (@GermanBluefox) Fixed: moving the enum "bad" also moved the enum "bad2", and cancelling the drag of an enum moved it to the top level of its category
- (@GermanBluefox) The buttons that remove an attribute in the object and host editors, and the buttons that close the details of adapter, instance and host cards, are ordinary grey close icons now, like the "Clear text" buttons. The red crosses and their rotation on hover are gone
