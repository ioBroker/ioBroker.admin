...
class ConfigSlider extends ConfigGeneric<ConfigSliderProps, ConfigSliderState> {
    ...
    static getDerivedStateFromProps(props: ConfigSliderProps, state: ConfigSliderState): Partial<ConfigSliderState> | null {
        ...
        if (_value === null || _value === undefined || _value.toString() !== parseFloat(state._value as any as string).toString()) {
            return { _value };
        }
    ...
    renderItem(error: string, disabled: boolean /* , defaultValue */) {
        ...
        return <Box style={styles.fullWidth}>
            {this.props.schema.label ? <Typography gutterBottom>{this.getText(this.props.schema.label)}</Typography> : null}
            <Slider
                style={styles.slider}
