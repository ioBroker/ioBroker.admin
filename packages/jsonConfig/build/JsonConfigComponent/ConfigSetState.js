...
class ConfigSetState extends ConfigGeneric {
    ...
    renderConfirmDialog() {
        ...
        return React.createElement(ConfirmDialog, { title: this.getText(confirm.title) || I18n.t('ra_Please confirm'), text: this.getText(confirm.text), ok: this.getText(confirm.ok) || I18n.t('ra_Ok'), cancel: this.getText(confirm.cancel) || I18n.t('ra_Cancel'), icon: icon, onClose: isOk => this.setState({ confirmDialog: false }, () => isOk && this._onClick()) });
    }
    renderItem(error, disabled /* , defaultValue */) {
        return React.createElement(Button, { variant: this.props.schema.variant || undefined, color: this.props.schema.color || 'grey', className: this.props.classes.fullWidth, disabled: disabled, onClick: async () => {
            ...
            this.props.schema.icon ? React.createElement(Icon, { src: this.props.schema.icon, className: this.props.classes.icon }) : null,
