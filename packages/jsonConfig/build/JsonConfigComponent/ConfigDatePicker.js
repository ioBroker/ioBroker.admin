...
class ConfigDatePicker extends ConfigGeneric {
    ...
    renderItem(error, disabled /* , defaultValue */) {
        return React.createElement(DatePicker, { fullWidth: true, margin: "normal", format: this.props.systemConfig.dateFormat.toLowerCase().replace('mm', 'MM'), error: !!error, disabled: !!disabled, value: this.state.value === null || this.state.value === undefined ? new Date() : this.state.value, KeyboardButtonProps: {
                'aria-label': 'change date',
            }, inputProps: { maxLength: this.props.schema.maxLength || this.props.schema.max || undefined }, onChange: value => {
