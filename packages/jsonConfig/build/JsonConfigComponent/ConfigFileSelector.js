...
import { InputLabel, MenuItem, FormHelperText, FormControl, Select, IconButton, ListItemText, ListItemIcon, } from '@mui/material';
import { Refresh as IconRefresh, UploadFile as IconUpload, Delete as IconDelete, PlayArrow as IconPlay, MusicNote as IconAudio, Videocam as IconVideo, Article as IconText, Code as IconCode, } from '@mui/icons-material';
...
class ConfigFileSelector extends ConfigGeneric {
    ...
    renderDeleteDialog() {
        ...
        return React.createElement(ConfirmDialog, { title: I18n.t('ra_Are you sure?'), text: I18n.t('ra_File will be deleted'), ok: I18n.t('ra_Delete'), cancel: I18n.t('ra_Cancel'), onClose: isOk => {
    ...
    getIcon(item) {
        ...
        if (IMAGE_EXT.includes(item.extension)) {
            return React.createElement("div", { className: this.props.classes.selectedImage, style: {
        ...
        if (AUDIO_EXT.includes(item.extension)) {
            return React.createElement(IconAudio, null);
        }
    ...
    renderItem(error, disabled /* , defaultValue */) {
        ...
        const element = React.createElement("div", { className: this.props.classes.fullWidth },
            React.createElement(FormControl, { variant: "standard", style: { width: `calc(100% - ${buttons * 42}px)` } },
                this.props.schema.label ? React.createElement(InputLabel, null, this.getText(this.props.schema.label)) : null,
                React.createElement(Select, { variant: "standard", error: !!error, disabled: !!disabled, value: this.state.value || '_', renderValue: () => React.createElement(React.Fragment, null,
                        this.getIcon(item),
                        React.createElement("span", null, item?.label || '')), onChange: e => {
                        this.setState({ value: e.target.value === '_' ? '' : e.target.value }, () => this.onChange(this.props.attr, this.state.value));
                    } }, selectOptions.map(it => React.createElement(MenuItem, { key: it.value, value: it.value },
                    React.createElement(ListItemIcon, null, this.getIcon(item)),
                    React.createElement(ListItemText, null, item.label),
                    this.props.schema.delete && item.value ?
                        React.createElement(IconButton, { className: this.props.classes.deleteButton, size: "small", onClick: () => this.setState({ deleteFile: item.value }) },
            ...
            this.props.schema.refresh && React.createElement(IconButton, { onClick: () => this.updateFiles() },
                React.createElement(IconRefresh, null)),
            this.props.schema.upload && React.createElement(IconButton, { onClick: () => this.dropzoneRef.current?.open() },
                React.createElement(IconUpload, null)),
            play && React.createElement(IconButton, { style: { color: '#00FF00' }, onClick: () => this.play() },
        ...
        return React.createElement(Dropzone, { ref: this.dropzoneRef, multiple: false, accept: accept, noKeyboard: true, noClick: true, maxSize: this.props.schema.maxSize || 2 * 1024 * 1024, onDragEnter: () => {
            ...
            } }, ({ getRootProps, getInputProps }) => React.createElement("div", { className: Utils.clsx(this.props.classes.uploadDiv, this.state.uploadFile === 'dragging' && this.props.classes.uploadDivDragging, disabled && this.props.classes.disabledOpacity), ...getRootProps() },
            React.createElement("input", { ...getInputProps() }),
            this.state.uploadFile === 'dragging' ? React.createElement("div", { className: Utils.clsx(this.props.classes.uploadCenterDiv, this.state.uploadError && this.props.classes.error) },
                React.createElement("div", { className: this.props.classes.uploadCenterTextAndIcon },
                    React.createElement(UploadIcon, { className: this.props.classes.uploadCenterIcon }),
                    React.createElement("div", { className: this.props.classes.uploadCenterText }, this.state.uploadFile === 'dragging' ? I18n.t('ra_Drop file here') :
                        I18n.t('ra_Place your files here or click here to open the browse dialog')))) : null,
            element,
            this.renderDeleteDialog()));
    }
}
