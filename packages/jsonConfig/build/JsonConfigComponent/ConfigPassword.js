...
class ConfigPassword extends ConfigGeneric {
    ...
    renderItem(error, disabled /* , defaultValue */) {
        ...
        const password = React.createElement(TextField, { variant: "standard", fullWidth: true, type: this.state._visible && this.state.value !== PASSWORD_PLACEHOLDER ? 'text' : 'password', value: this.state.value, error: !!error || this.state._notEqual, disabled: !!disabled, onChange: e => this.onChangePassword(e.target.value), label: this.getText(this.props.schema.label), inputProps: {
            ...
            InputProps: {
                endAdornment: this.state.value && this.state.value !== PASSWORD_PLACEHOLDER && this.props.schema.visible ? React.createElement(InputAdornment, { position: "end" },
        ...
        if (this.props.schema.repeat) {
            const passwordRepeat = React.createElement(TextField, { variant: "standard", fullWidth: true, type: this.state._visible && this.state._repeat !== PASSWORD_PLACEHOLDER ? 'text' : 'password', value: this.state._repeat, error: !!error || this.state._notEqual, disabled: !!disabled, onChange: e => this.onChangePassword(undefined, e.target.value), label: `${this.getText(this.props.schema.label)} (${I18n.t('ra_repeat')})`, inputProps: {
                    autoComplete: 'new-password',
                    form: { autoComplete: 'off' },
                    maxLength: this.props.schema.maxLength || this.props.schema.max || undefined,
                }, helperText: this.state._notEqual ? I18n.t('ra_Passwords are not equal!') : this.renderHelp(this.props.schema.help, this.props.schema.helpLink, this.props.schema.noTranslation), 
                // eslint-disable-next-line react/jsx-no-duplicate-props
                InputProps: {
                    endAdornment: this.state._repeat && this.state._repeat !== PASSWORD_PLACEHOLDER ? React.createElement(InputAdornment, { position: "end" },
                        React.createElement(IconButton, { size: "large", tabIndex: -1, onClick: e => {
                                e.preventDefault();
                                e.stopPropagation();
                                this.setState({ _visible: !this.state._visible });
            ...
            return React.createElement("div", { className: this.props.classes.fullWidth },
                React.createElement("div", { className: this.props.classes.halfWidth1 }, password),
                React.createElement("div", { className: this.props.classes.halfWidth2 }, passwordRepeat));
        }
        return password;
    }
}
