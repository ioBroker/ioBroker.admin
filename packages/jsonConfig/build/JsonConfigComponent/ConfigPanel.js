...
class ConfigPanel extends ConfigGeneric {
    ...
    renderItems(items, disabled) {
        ...
        return items ? Object.keys(items).map(attr => {
            ...
            return React.createElement(ItemComponent, { key: `${attr}_${this.props.index === undefined ? '' : this.props.index}`, index: this.props.index, changed: this.props.changed, arrayIndex: this.props.arrayIndex, globalData: this.props.globalData, onCommandRunning: this.props.onCommandRunning, commandRunning: this.props.commandRunning, className: classes.panel, socket: this.props.socket, adapterName: this.props.adapterName, instance: this.props.instance, common: this.props.common, customs: this.props.customs, alive: this.props.alive, themeType: this.props.themeType, themeName: this.props.themeName, data: this.props.data, originalData: this.props.originalData, systemConfig: this.props.systemConfig, onError: this.props.onError, onChange: this.props.onChange, multiEdit: this.props.multiEdit, dateFormat: this.props.dateFormat, isFloatComma: this.props.isFloatComma, disabled: disabled, imagePrefix: this.props.imagePrefix, changeLanguage: this.props.changeLanguage, forceUpdate: this.props.forceUpdate, registerOnForceUpdate: this.props.registerOnForceUpdate, customObj: this.props.customObj, instanceObj: this.props.instanceObj, custom: this.props.custom, schema: items[attr], attr: attr });
        }) : null;
    }
    render() {
        ...
        if (this.props.custom) {
            return React.createElement(Grid, { key: `${this.props.attr}_${this.props.index}`, container: true, className: classes.fullWidth, spacing: 2, style: style }, this.renderItems(items, disabled));
        }
        let content;
        if (schema.collapsable) {
            content = React.createElement(Accordion, { key: `${this.props.attr}_${this.props.index}`, className: classes.fullWidth, expanded: !!this.state.expanded, onChange: () => {
                ...
                React.createElement(AccordionSummary, { expandIcon: React.createElement(ExpandMoreIcon, null), style: ({ ...schema.style, ...(this.props.themeType ? schema.darkStyle : {}) }), className: Utils.clsx(classes.fullWidth, schema.color === 'primary' && classes.primary, schema.color === 'secondary' && classes.secondary) },
                    React.createElement(Typography, { className: classes.heading }, this.getText(schema.label))),
                React.createElement(AccordionDetails, null,
                    React.createElement(Grid, { container: true, className: `${classes.fullWidth} ${classes.padding}`, spacing: 2, style: style }, this.renderItems(items, disabled))));
        }
        else {
            content = React.createElement("div", { key: `${this.props.attr}_${this.props.index}`, className: Utils.clsx(this.props.className, this.props.isParentTab && classes.paper, classes.fullWidth), style: style },
                React.createElement(Grid, { container: true, className: Utils.clsx(classes.fullWidth, this.props.isParentTab && classes.padding), spacing: 2 }, this.renderItems(items, disabled)));
        }
        if (!this.props.isParentTab) {
            const item = React.createElement(Grid, { item: true, title: this.getText(schema.tooltip), xs: schema.xs || undefined, lg: schema.lg || undefined, md: schema.md || undefined, sm: schema.sm || undefined, style: ({ marginBottom: 0, /* marginRight: 8, */ textAlign: 'left', ...schema.style }) }, content);
            if (schema.newLine) {
                return React.createElement(React.Fragment, null,
                    React.createElement("div", { style: { flexBasis: '100%', height: 0 } }),
                    item);
