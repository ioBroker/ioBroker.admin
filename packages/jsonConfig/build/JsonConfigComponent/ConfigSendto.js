...
class ConfigSendto extends ConfigGeneric {
    ...
    renderErrorDialog() {
        if (this.state._error) {
            ...
            return React.createElement(DialogError, { text: this.state._error, classes: undefined, onClose: () => this.setState({ _error: '' }) });
        }
        return null;
    }
    renderMessageDialog() {
        if (this.state._message) {
            return React.createElement(DialogMessage, { text: this.state._message, onClose: () => this.setState({ _message: '' }) });
        }
        return null;
    }
    _onClick() {
    ...
    renderConfirmDialog() {
        ...
        return React.createElement(DialogConfirm, { title: this.getText(confirm.title) || I18n.t('ra_Please confirm'), text: this.getText(confirm.text), ok: this.getText(confirm.ok) || I18n.t('ra_Ok'), cancel: this.getText(confirm.cancel) || I18n.t('ra_Cancel'), icon: icon, onClose: isOk => this.setState({ confirmDialog: false }, () => isOk && this._onClick()) });
    }
    renderItem(error, disabled /* , defaultValue */) {
        const icon = this.getIcon();
        return React.createElement("div", { className: this.props.classes.fullWidth },
            React.createElement(Button, { variant: this.props.schema.variant || undefined, color: this.props.schema.color || 'grey', className: this.props.classes.fullWidth, disabled: disabled || !this.props.alive, startIcon: icon, title: this.props.alive ? this.getText(this.props.schema.title) || '' : I18n.t('ra_Instance is not alive'), onClick: () => {
