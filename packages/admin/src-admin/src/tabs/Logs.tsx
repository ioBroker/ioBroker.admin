...
class Logs extends Component<LogsProps, LogsState> {
    ...
    getLogFiles() {
        return this.state.logFiles.map(entry => (
            ...
            >
                {entry.name}
                <Typography style={styles.downloadLogSize} variant="caption">
    ...
    getSeverities() {
        ...
        for (const i in this.severities) {
            severities.push(
                <MenuItem value={i} key={i} style={styles[`${this.props.themeType}_${i}`]}>
                    {i}
                </MenuItem>
            );
        }
    ...
    getSources() {
        ...
        return sources.map(id => (
            ...
            >
                {id === '1' ? null : <Icon src={this.state.sources[id].icon || ''} style={styles.iconSelect} />}
    ...
    renderClearDialog() {
        ...
        return (
            <Dialog onClose={() => this.setState({ logDeleteDialog: false })} open={!0}>
                <DialogTitle>
    ...
    renderToolbar() {
        ...
        const downloadLogButton = isMobile ? (
            <IconButton color="primary" onClick={event => this.setState({ logDownloadDialog: event.currentTarget })}>
                <SaveAltIcon />
            </IconButton>
        ) : (
            <Button
        ...
        return (
            <TabHeader>
                ...
                <Tooltip title={this.props.t('Pause output')} slotProps={{ popper: { sx: styles.tooltip } }}>
                    <IconButton size="large" style={styles.pauseButton} onClick={() => this.handleLogPause()}>
                        {pauseChild}
                    </IconButton>
                </Tooltip>
