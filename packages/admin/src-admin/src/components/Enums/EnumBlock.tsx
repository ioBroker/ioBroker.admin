...
class EnumBlock extends Component<EnumBlockProps, EnumBlockState> {
    ...
    render() {
        ...
        icon = props.childrenCount ? (
            <div style={styles.folderDiv} onClick={() => props.toggleEnum(props.id)}>
                {props.closed
                    ? [<IconCollapsed style={styles.folder} key={1} />, <div key={2}>{icon}</div>]
                    : [<IconExpanded style={styles.folder} key={1} />, <div key={2}>{icon}</div>]}
            </div>
        ) : (
            icon
        );
            ...
            >
                <div style={styles.enumCardContent}>
                    <div style={styles.right}>
                        {props.enum ? (
                            <IconButton size="small" onClick={() => props.showEnumEditDialog(props.enum, false)}>
                                <Tooltip
                                    title={props.t('Edit')}
                                    placement="top"
                        ...
                        {props.enum ? (
                            <IconButton size="small" onClick={() => props.copyEnum(props.id)}>
                                <Tooltip
                                    title={props.t('Clone')}
                                    placement="top"
                                    slotProps={{ popper: { sx: { pointerEvents: 'none' } } }}
