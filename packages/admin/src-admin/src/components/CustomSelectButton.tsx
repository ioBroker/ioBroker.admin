    ...
    return (
        <>
            <Tooltip title={title || ''} slotProps={{ popper: { sx: { pointerEvents: 'none' } } }}>
                ...
                >
                    {buttonIcon ||
                        (icons && <MaterialDynamicIcon objIconBool iconName={value as string} style={styles.icon} />)}
            ...
            <Menu anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
                {arrayItem.map(({ name }, idx) => (
                    ...
                    >
                        {icons && <MaterialDynamicIcon objIconBool iconName={name as string} style={styles.icon} />}
