...
> extends Component<TProps, TState> {
    ...
    renderDeleteDialog() {
        return (
            <Dialog onClose={() => this.setState({ openDialogDelete: false, openDialog: false })} open={!0}>
    ...
    renderTooltip() {
        ...
        return [
            ...
            instance.mode === 'daemon' && this.props.item.stoppedWhenWebExtension === undefined ? (
                <State key={2} state={item.connectedToHost}>
            ...
            this.props.item.connected !== null && this.props.item.stoppedWhenWebExtension === undefined ? (
                <State key={4} state={!!item.connected}>
            ...
            cronText ? (
                <State key={5} state={instance.enabled}>
                    CRON:
                    {cronText}
                </State>
            ) : null,
    ...
    renderMemoryUsage() {
        return (
            this.props.item.running && (
                <InstanceInfo icon={<MemoryIcon />} tooltip={this.props.context.t('RAM usage')}>
    ...
    renderPlayPause() {
        return (
            <Tooltip title={this.props.context.t('Start/stop')} slotProps={{ popper: { sx: this.styles.tooltip } }}>
                <div>
                    <IconButton
                        size="small"
                        onClick={event => {
    ...
    renderRestartButton() {
        return (
            <Tooltip title={this.props.context.t('Restart')} slotProps={{ popper: { sx: this.styles.tooltip } }}>
                <div>
                    <IconButton
                        size="small"
                        onClick={event => {
    ...
    renderInputOutput() {
        return (
            <InstanceInfo icon={<ImportExportIcon />} tooltip={this.props.context.t('events')}>
    ...
    renderCompactGroupEnabled() {
        return (
            <Tooltip title={this.props.context.t('compact groups')} slotProps={{ popper: { sx: this.styles.tooltip } }}>
                <IconButton
                    size="small"
                    style={{
                        ...this.styles.button,
    ...
    renderDeleteButton() {
        return (
            <Tooltip title={this.props.context.t('Delete')} slotProps={{ popper: { sx: this.styles.tooltip } }}>
                <IconButton
                    size="small"
                    style={this.styles.button}
                    onClick={async event => {
